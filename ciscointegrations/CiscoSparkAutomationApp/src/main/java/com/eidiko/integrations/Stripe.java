...
public class Stripe implements Runnable {
	...
	final static Logger logs = Logger.getLogger(Stripe.class);
	...
	public Stripe(Properties prop) {
		this.prop = prop;
		xls = new ReadExcelData(prop.getProperty("Stripefile"));
...
//
		...
		//	logs.info("Select repositories");
			...
			try {
				...
				FileUtils.copyFile(scrFile, new File(screenshot+"/Stripe"+ sdf.format(d) + ".png"));
				PostMessage.restCall("Unable to add and remove Stripe integration", prop);
				driver.close();
			} catch (Exception e1) {
	...
	public void authorizeStripe() throws Exception{
		try{
			// Enter Stripe email id
			logString = "Enter Stripe email id";
			logs.info(logString);
			commons.allClick(xls.getData(3, 1)).sendKeys(xls.getData(3, 2));

			// Enter Stripe password
			logString = "Enter Stripe password";
			logs.info(logString);
